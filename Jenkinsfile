node {
  ...
  sh 'ssh ubuntu@ec2-54-213-251-190.us-west-2.compute.amazonaws.com "git clone https://github.com/nburk2/home_auto.git; cd home_auto; git checkout develop; npm install; gulp;sudo nohup ~/home_auto_server/tomcat/bin/shutdown.sh; sudo rm -r ~/home_auto_server/tomcat/webapps/ROOT/*; sudo mkdir ~/home_auto_server/tomcat/webapps/ROOT; sudo mv dist/** ~/home_auto_server/tomcat/webapps/ROOT;sudo nohup ~/home_auto_server/tomcat/bin/startup.sh > foo.out 2> foo.err < /dev/null &;cd ..;sudo rm -rf home_auto"'
